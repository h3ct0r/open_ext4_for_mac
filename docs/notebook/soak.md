...
| 2026-09-05 | 5 (+fuzz) | second run with `--fuzz 10`, harness fixed: five clean rounds, the corpus merged before every pass and staying under budget, no OOM artifacts. Round 5's read-only fuzz pass found a real one: `ext4_xattr_list` packs pointer-holding list entries straight after each name, so any inode with two short-named attributes puts the second entry on an odd address -- undefined behaviour ARM64 tolerates, UBSan reports, and a healthy volume reproduces. Patch 0079, fixture 0021, a bounds cell red-first under the sanitizer (57/1 → 58/0). Count restarted. |
| 2026-09-05 | 2 (+fuzz) | third run: two clean rounds, then round 2's read-only fuzz pass produced a `timeout-` artifact -- not a hang, a 52-second walk: a superblock claiming 4,294,967,295 inodes in one group of 176, which the gate accepted because fixture 0020's rule checked only the too-small side. The Linux kernel refuses both sides ("inodes count not valid: N vs M"); the gate now requires equality, and the harness bounds its whole-file extent walk to 16 MiB at each end. Fixture 0022. Count restarted. |
| 2026-09-05 | **20** (+fuzz) | fourth run, on `ba71e40`, 07:01 to 17:07: twenty clean rounds of the full 29-stage set with ten minutes of fuzzing in each mode between rounds. Rounds took 582–607 s, except round 19 at 1,355 s while the machine was compiling other software and building Docker images alongside it (round 20 was back to 588 s). Round 1's fuzz pass produced three `oom-` artifacts; the triage re-ran each alone, none reproduced, and the soak carried on -- the first time that path ran for real. No artifact after that. The corpora, merged before every pass, ended at 659 MB read-only and 417 MB read-write. The first soak with fuzzing on to reach its count; the three earlier attempts each stopped on a finding (the harness's memory budget, patch 0079, fixture 0022), all fixed red-first before this one began. |
