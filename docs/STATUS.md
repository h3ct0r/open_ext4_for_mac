...
- A notification when a locked volume appears and the agent is not running:
  the extension records it and `Ext4Mac status` / `Ext4Mac last-error` show
  it, but the notification itself needs the agent (see "What the extension
  says when it will not mount")
