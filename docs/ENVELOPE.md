...
| formatting a physical disk | **yes**, from Disk Utility, `diskutil eraseVolume` or `newfs_fskit` | It failed until 2026-09-06 with `-69832`: the `.fs` wrapper re-dispatched the work to the console user, who cannot open a physical disk's `root:operator` node, so `fskitd` answered `EACCES` before this driver was asked. The wrapper now keeps the root `storagekitd` gave it and sets `SUDO_UID`, which makes `newfs_fskit` call `setreuid(SUDO_UID, -1)` — real uid the console user, so the per-user module is found; effective uid root, so the device opens. Nothing is chowned and no prompt appears |
