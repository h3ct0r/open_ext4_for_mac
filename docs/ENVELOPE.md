...
| formatting a physical disk | **not through FSKit** | macOS runs `newfs_fskit`'s work as the console user, and a physical disk's node is `root:operator` mode 0640 — so `fskitd` answers `EACCES` (`errno` 13) before this driver is asked, and Disk Utility reports `-69832`. Being an administrator does not help: `operator` grants read, not write. Disk images format normally. Measured 2026-09-05 and again 2026-09-06 |
