...
| formatting a physical disk | **refused unless you own the device node** | macOS runs `newfs_fskit`'s work as the console user, and a physical disk's node is `root:operator` mode 0640 — so `fskitd` answers `EACCES` (`errno` 13) before this driver is asked, and Disk Utility reports `-69832`. Being an administrator does not help: `operator` grants read, not write. `sudo chown $(id -u) /dev/diskNsM /dev/rdiskNsM` first and the same erase succeeds, mounts immediately and passes `e2fsck -fn` — measured 2026-09-06 on a 256 GB stick. Ownership reverts on replug or reboot. Disk images format normally, since their nodes belong to whoever attached them |
